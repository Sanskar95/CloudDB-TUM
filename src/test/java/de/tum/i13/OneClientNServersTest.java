...
public class OneClientNServersTest {
    ...
    private static void launchServer(boolean test) throws IOException, InterruptedException {
        ...
        for (int i = 0; i<nioThreads.length; i++){
            int incrementer = i;
            nioThreads[i] = new Thread() {
                @Override
                public void run() {
                    try {
                        StartSimpleNioServer.main(new String[]{"-p",(NIO_PORT + (incrementer*10))+"","-d","data/" + incrementer + "/","-b",HOST+":"+ECS_PORT });
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
